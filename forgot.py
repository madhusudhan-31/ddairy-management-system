...
class ResetPassword(ctk.CTk):
    ...
    def reset_password(self):
        ...
        try:
            ...
            cursor.execute("UPDATE admin SET pasword = %s WHERE mail = %s", (new_password, email))
            conn.commit()

            if cursor.rowcount == 0:
                self.status_label.configure(text="password allready exists for for this mail", text_color="red")
            else:
